---
title: "Getting started with prodigenr/ prodigenrCCMS"
author: "Luke W. Johnston, adapted to CCMS needs by Anna Schritz"
date: "`r Sys.Date()`"
...
prodigenrCCMS is an adapted package of the original package prodigenr to the needs of CCMS.
...
## Setting up a project with prodigenrCCMS

Starting a research project? Create a project directory like so:

```{r manuscript-proj}
# install.packages(devtools) # only needs to be installed once
# install_github("ccmsLIH/projPropCCMS") # only needs to be installed once

library(devtools)
library(projPropCCMS)


prodigenrCCMS::setup_project("path/YourProjectName")
```

Or via RStudio's interface (only for prodigenr, not projPropCCMS) (with RStudio version >1.1):
...
projPropCCMS:::viz_project_tree(new_project_path)
```
...
- `SAS`: Should contain the SAS scripts and macros used for the analysis.
...
- `datamanagement/`: A folder for all data management files
...
depends on.
- `literature/`: If relevant, all useful literature references can be put here.
...
At present, there are only three template files (SAP added for CCMS) that you can view:
